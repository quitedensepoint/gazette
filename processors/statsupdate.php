...
$taa = mysqli_query($dbConnComm, "SELECT w.callsign,
                                  ...
                                  GROUP BY w.callsign ORDER BY points DESC limit 100") or die ($dbConnComm->error.taaaquery_LINE_);
/* TOP AAA */
...
$dtaa = mysqli_query($dbConnComm, "SELECT w.callsign,
...
$tatg = mysqli_query($dbConnComm, "SELECT w.callsign, 
                                            IF(SUM(CASE s.rtb WHEN 3 THEN 1 ELSE 0 END)>0, SUM(s.kills) * ROUND(SUM(s.kills)/(SUM(CASE s.rtb WHEN 3 THEN 1 ELSE 0 END)),2), SUM(s.kills)) AS points 
                                    FROM scoring_campaign_sorties s
                                    JOIN wwii_player w ON s.player_id=w.playerid, scoring_persona_configs c  
                                        ...
                                        AND s.persona_id=c.persona_id
                                        AND (isnull(c.bans) OR c.bans = 0)
                                    GROUP BY w.callsign ORDER BY `points` DESC limit 100") or die ($dbConnComm->error.tatg_LINE_);
/* TOP ATG */
...
$dtatg = mysqli_query($dbConnComm, "SELECT w.callsign, 
                                            IF(SUM(CASE s.rtb WHEN 3 THEN 1 ELSE 0 END)>0, SUM(s.kills) * ROUND(SUM(s.kills)/(SUM(CASE s.rtb WHEN 3 THEN 1 ELSE 0 END)),2), SUM(s.kills)) AS points 
                                    FROM scoring_campaign_sorties s
                                    JOIN wwii_player w ON s.player_id=w.playerid 
                                        WHERE s.vehicle_id IN(156,124,207,202,69,17,18,154,7,123)
                                        AND DATE(s.`sortie_start`) >= DATE_SUB(NOW(), INTERVAL 24 HOUR)
                                    GROUP BY w.callsign ORDER BY `points` DESC limit 100") or die ($dbConnComm->error.datg_LINE_);
/* DAILY TOP ATG */
...
$tatr = mysqli_query($dbConnComm, "SELECT w.callsign, 
                                    ...
                                    JOIN wwii_player w ON s.player_id=w.playerid, scoring_persona_configs c  
                                        ...
                                        AND s.persona_id=c.persona_id
                                        AND (isnull(c.bans) OR c.bans = 0)
                                    GROUP BY w.callsign ORDER BY `points` DESC limit 100") or die ($dbConnComm->error.atr_LINE_);
/* TOP ATR */
...
$dtatr = mysqli_query($dbConnComm, "SELECT w.callsign, 
                                            IF(SUM(CASE s.rtb WHEN 3 THEN 1 ELSE 0 END)>0, (SUM(s.kills)+SUM(s.captures)) * ROUND(SUM(s.kills)/(SUM(CASE s.rtb WHEN 3 THEN 1 ELSE 0 END)),2), SUM(s.kills)) AS points 
                                    FROM scoring_campaign_sorties s
                                    JOIN wwii_player w ON s.player_id=w.playerid  
                                        WHERE s.vehicle_id IN(120,121,166,172,260)
                                        AND DATE(s.`sortie_start`) >= DATE_SUB(NOW(), INTERVAL 24 HOUR)
                                    GROUP BY w.callsign ORDER BY `points` DESC limit 100") or die ($dbConnComm->error.datr_LINE_);
/* DAILY TOP ATR */
...
$teng = mysqli_query($dbConnComm, "SELECT w.callsign, 
                                    ...
                                    JOIN wwii_player w ON s.player_id=w.playerid, scoring_persona_configs c  
                                        ...
                                        AND s.persona_id=c.persona_id
                                        AND (isnull(c.bans) OR c.bans = 0)
                                    GROUP BY w.callsign ORDER BY `points` DESC limit 100") or die ($dbConnComm->error.engquery_LINE_);
/* TOP Engineer */
...
$dteng = mysqli_query($dbConnComm, "SELECT w.callsign, 
                                              IF(SUM(CASE s.rtb WHEN 3 THEN 1 ELSE 0 END)>0, (SUM(s.kills)+(SUM(s.captures)*5)) * ROUND(SUM(s.kills)/(SUM(CASE s.rtb WHEN 3 THEN 1 ELSE 0 END)),2), SUM(s.kills)) AS points 
                                    FROM scoring_campaign_sorties s
                                    JOIN wwii_player w ON s.player_id=w.playerid  
                                        WHERE s.vehicle_id IN(10,21,205,265)
                                        AND DATE(s.`sortie_start`) >= DATE_SUB(NOW(), INTERVAL 24 HOUR)
                                    GROUP BY w.callsign ORDER BY `points` DESC limit 100") or die ($dbConnComm->error.dengquery_LINE_);
/* DAILY TOP Engineer */
...
$tgren = mysqli_query($dbConnComm, "SELECT w.callsign, 
                                    ...
                                    JOIN wwii_player w ON s.player_id=w.playerid, scoring_persona_configs c  
                                        ...
                                        AND s.persona_id=c.persona_id
                                        AND (isnull(c.bans) OR c.bans = 0)
                                    GROUP BY w.callsign ORDER BY `points` DESC limit 100") or die ($dbConnComm->error.grenadierquery_LINE_);
/* TOP GRENADIER */
...
$dtgren = mysqli_query($dbConnComm, "SELECT w.callsign, 
...
$tlmg = mysqli_query($dbConnComm, "SELECT w.callsign, 
                                    ...
                                    JOIN wwii_player w ON s.player_id=w.playerid, scoring_persona_configs c  
                                        ...
                                        AND s.persona_id=c.persona_id
                                        AND (isnull(c.bans) OR c.bans = 0)
                                    GROUP BY w.callsign ORDER BY `points` DESC limit 100") or die ($dbConnComm->error.lmgquery_LINE_);
/* TOP LMG */
...
$dtlmg = mysqli_query($dbConnComm, "SELECT w.callsign, 
...
$tmort = mysqli_query($dbConnComm, "SELECT w.callsign, 
                                        IF(SUM(CASE s.rtb WHEN 3 THEN 1 ELSE 0 END)>0, (SUM(s.kills)+(SUM(s.captures)*2)) * ROUND(SUM(s.kills)/(SUM(CASE s.rtb WHEN 3 THEN 1 ELSE 0 END)),2), SUM(s.kills)) AS points 
                                    FROM scoring_campaign_sorties s
                                    JOIN wwii_player w ON s.player_id=w.playerid, scoring_persona_configs c  
                                        ...
                                        AND s.persona_id=c.persona_id
                                        AND (isnull(c.bans) OR c.bans = 0)
                                    GROUP BY w.callsign ORDER BY `points` DESC limit 100") or die ($dbConnComm->error.mortquery_LINE_);
/* TOP MORTAR */
...
$dtmort = mysqli_query($dbConnComm, "SELECT w.callsign, 
...
$trifle = mysqli_query($dbConnComm, "SELECT w.callsign, 
                                    ...
                                    JOIN wwii_player w ON s.player_id=w.playerid, scoring_persona_configs c  
                                        ...
                                        AND s.persona_id=c.persona_id
                                        AND (isnull(c.bans) OR c.bans = 0)
                                    GROUP BY w.callsign ORDER BY `points` DESC limit 100") or die ($dbConnComm->error.testriflequery_LINE_);
/* TOP RIFLE */
...
$dtrifle = mysqli_query($dbConnComm, "SELECT w.callsign, 
...
$tsmg = mysqli_query($dbConnComm, "SELECT w.callsign, 
                                    ...
                                    JOIN wwii_player w ON s.player_id=w.playerid, scoring_persona_configs c  
                                        ...
                                        AND s.persona_id=c.persona_id
                                        AND (isnull(c.bans) OR c.bans = 0)
                                    GROUP BY w.callsign ORDER BY `points` DESC limit 100") or die ($dbConnComm->error.smgquery_LINE_);
/* TOP SMG */
...
$dtsmg = mysqli_query($dbConnComm, "SELECT w.callsign, 
...
$tsniper = mysqli_query($dbConnComm, "SELECT w.callsign, 
...
$dtsniper = mysqli_query($dbConnComm, "SELECT w.callsign, 
...
$ttank = mysqli_query($dbConnComm, "SELECT w.callsign, 
                                    ...
                                    JOIN wwii_player w ON s.player_id=w.playerid, scoring_persona_configs c  
                                        ...
                                        AND s.persona_id=c.persona_id
                                        AND (isnull(c.bans) OR c.bans = 0)
                                    GROUP BY w.callsign ORDER BY `points` DESC limit 100") or die ($dbConnComm->error.tankquery_LINE_);
/* TOP TANK */
...
$dttank = mysqli_query($dbConnComm, "SELECT w.callsign, 
...
$ttruck = mysqli_query($dbConnComm, "SELECT w.callsign, 
                                    ...
                                    JOIN wwii_player w ON s.player_id=w.playerid, scoring_persona_configs c  
                                        ...
                                        AND s.persona_id=c.persona_id
                                        AND (isnull(c.bans) OR c.bans = 0) 
                                    GROUP BY w.callsign ORDER BY `points` DESC limit 100") or die ($dbConnComm->error.truckquery_LINE_);
/* TOP TRUCK */
...
$dttruck = mysqli_query($dbConnComm, "SELECT w.callsign, 
                                        COUNT(CASE WHEN s.player_id = '*' THEN 1 ELSE 0 END) as sortie,
                                        IF(SUM(CASE s.rtb WHEN 3 THEN 1 ELSE 0 END)>0, SUM(s.kills) * ROUND(SUM(s.kills)/(SUM(CASE s.rtb WHEN 3 THEN 1 ELSE 0 END)),2), SUM(s.kills)) AS points  
                                    FROM scoring_campaign_sorties s
                                    JOIN wwii_player w ON s.player_id=w.playerid  
...
$tfight = mysqli_query($dbConnComm, "SELECT w.callsign, 
                                    ...
                                    JOIN wwii_player w ON s.player_id=w.playerid, scoring_persona_configs c  
                                        ...
                                        AND s.persona_id=c.persona_id
                                        AND (isnull(c.bans) OR c.bans = 0)
                                    GROUP BY w.callsign ORDER BY `points` DESC limit 100") or die ($dbConnComm->error.bomber_LINE_);
/* TOP FIGHTER */
...
$dtfight = mysqli_query($dbConnComm, "SELECT w.callsign, 
...
$tbomb = mysqli_query($dbConnComm, "SELECT w.callsign, 
...
$dtbomb = mysqli_query($dbConnComm, "SELECT w.callsign, 
...
$tdd = mysqli_query($dbConnComm, "SELECT w.callsign, 
                                    ...
                                    JOIN wwii_player w ON s.player_id=w.playerid, scoring_persona_configs c  
                                        ...
                                        AND s.persona_id=c.persona_id
                                        AND (isnull(c.bans) OR c.bans = 0)
                                    GROUP BY w.callsign ORDER BY `points` DESC limit 100") or die ($dbConnComm->error.dd_LINE_);
/* TOP DESTROYER */
...
$dtdd = mysqli_query($dbConnComm, "SELECT w.callsign, 
...
$tpb = mysqli_query($dbConnComm, "SELECT w.callsign, 
                                    ...
                                    JOIN wwii_player w ON s.player_id=w.playerid, scoring_persona_configs c  
                                        ...
                                        AND s.persona_id=c.persona_id
                                        AND (isnull(c.bans) OR c.bans = 0)
                                    GROUP BY w.callsign ORDER BY `points` DESC limit 100") or die ($dbConnComm->error.pb_LINE_);
/* TOP PATROL BOATS */
...
$dtpb = mysqli_query($dbConnComm, "SELECT w.callsign, 
...
$ttt = mysqli_query($dbConnComm, "SELECT w.callsign, 
                                    ...
                                    JOIN wwii_player w ON s.player_id=w.playerid, scoring_persona_configs c  
                                        ...
                                        AND s.persona_id=c.persona_id
                                        AND (isnull(c.bans) OR c.bans = 0) 
                                    GROUP BY w.callsign ORDER BY `points` DESC limit 100") or die ($dbConnComm->error.ttquery_LINE_);
...
$dttt = mysqli_query($dbConnComm, "SELECT w.callsign, 
...
$allykills = mysqli_query($dbConnComm, "SELECT callsign, 
                                        ...
                                        WHERE wwii_persona.personaid = p.persona_id 
                                            ...
                                            AND (ISNULL(c.bans) OR c.bans = 0) 
                                        GROUP BY callsign ORDER BY kills DESC LIMIT 100") or die ($dbConnComm->error.kills_LINE_);
/* TOP KILLS  */
...
$dallykills = mysqli_query($dbConnComm, "SELECT callsign, 
                                            SUM(s.kills) AS kills
                                        ...
                                        WHERE DATE(s.`sortie_start`) >= DATE_SUB(NOW(), INTERVAL 24 HOUR) 
                                            ...
                                            AND wwii_player.playerid = wwii_persona.playerid 
                                            AND wwii_persona.countryid !='4' 
                                            AND (ISNULL(c.bans) OR c.bans = 0) GROUP BY callsign ORDER BY kills DESC LIMIT 100") or die ($dbConnComm->error.kills_LINE_);
/* DAILY TOP KILLS */
...
/*  top captures info */
$allycaps = mysqli_query($dbConnComm, "SELECT callsign, 
                                       ...
                                       WHERE wwii_persona.personaid = p.persona_id 
                                            ...
                                            AND wwii_persona.countryid ='4' 
                                            AND (ISNULL(c.bans) OR c.bans = 0) 
                                       GROUP BY callsign ORDER BY caps DESC LIMIT 100") or die ($dbConnComm->error.caps_LINE_);
/* MOST CAPTURES */
...
$dallycaps = mysqli_query($dbConnComm, "SELECT callsign, SUM(s.captures) AS caps 
                                        FROM scoring_campaign_sorties AS s 
                                        LEFT JOIN scoring_persona_configs c ON (s.persona_id = c.persona_id),wwii_persona, wwii_player 
                                        WHERE DATE(s.`sortie_start`) >= DATE_SUB(NOW(), INTERVAL 24 HOUR) 
                                            AND wwii_persona.personaid = s.persona_id 
                                            ...
                                            AND wwii_persona.countryid !='4' 
                                            AND (ISNULL(c.bans) OR c.bans = 0) 
                                        GROUP BY callsign ORDER BY caps DESC LIMIT 100") or die ($dbConnComm->error.dcaps_LINE_);
/* DAILY MOST CAPTURES */
...
/* Kills/Death (Kd) */
$allykd = mysqli_query($dbConnComm, "SELECT callsign, SUM(p.kills) as kills, SUM(p.deaths) as deaths, 
                                        SUM(p.kd) as kd
                                     ...
                                     WHERE wwii_persona.personaid = p.persona_id 
                                            ...
                                            AND wwii_persona.countryid !='4' 
                                            AND (isnull(c.bans) OR c.bans = 0) 
                                    GROUP BY callsign ORDER BY kd DESC LIMIT 100") or die ($dbConnComm->error.kdget_LINE_);
/* Top K/D */
...
$dallykd = mysqli_query($dbConnComm, "SELECT callsign, 
...
$allytom = mysqli_query($dbConnComm, "SELECT callsign, p.tom AS tom 
                                      FROM scoring_campaign_personas p 
                                      LEFT JOIN scoring_persona_configs c ON (p.persona_id = c.persona_id), wwii_persona, wwii_player 
                                      WHERE wwii_persona.personaid = p.persona_id 
                                        AND wwii_player.playerid = wwii_persona.playerid 
                                        AND wwii_persona.countryid !='4' 
                                        AND (isnull(c.bans) OR c.bans = 0) 
                                      GROUP BY callsign ORDER BY tom DESC LIMIT 100") or die ($dbConnComm->error.tomget_LINE_);
/* TIME ON MISSION */
...
$dallytom = mysqli_query($dbConnComm, "SELECT callsign, SUM(s.tom) AS tom 
                                       FROM scoring_campaign_sorties s 
                                       LEFT JOIN scoring_persona_configs c ON (s.persona_id = c.persona_id), wwii_persona, wwii_player 
                                       WHERE DATE(s.sortie_start) = DATE_SUB(CURDATE(), INTERVAL 0 DAY ) 
                                            ...
                                            AND wwii_persona.countryid !='4' 
                                            AND (isnull(c.bans) OR c.bans = 0) 
                                       GROUP BY callsign ORDER BY tom DESC LIMIT 100") or die ($dbConnComm->error.dtomget_LINE_);
...
$tkrow = mysqli_query($dbConnComm, "SELECT callsign, streak_id, best as value2, UNIX_TIMESTAMP(achieved), v.name, 0, 0 
                                    ...
                                    WHERE v.vehicle_id=s.vehicle_id 
                                        ...
                                        AND s.streak_id = 4 
                                        AND wwii_persona.countryid !='4'  
                                        AND (isnull(c.bans) OR c.bans = 0) ORDER BY value2 DESC LIMIT 100") or die ($dbConnComm->error.tkrowquery_LINE_);
...
$dtkrow = mysqli_query($dbConnComm, "SELECT DISTINCT(callsign) as callsign, streak_id, current as value2 
                                    FROM scoring_campaign_streaks s, wwii_player, wwii_persona 
                                    WHERE s.streak_id = 4
                                        AND DATE(s.achieved) >= DATE_SUB(NOW(), INTERVAL 24 HOUR)
                                        AND wwii_persona.personaid = s.persona_id
                                        AND wwii_player.playerid = wwii_persona.playerid 
                                        ORDER BY value2 DESC LIMIT 100") or die ($dbConnComm->error.tkrowquery_LINE_);
...
$tcrow = mysqli_query($dbConnComm, "SELECT callsign, streak_id, best as value2, UNIX_TIMESTAMP(achieved), v.name, 0, 0 
                                    FROM scoring_vehicles v, scoring_campaign_streak_bests s 
                                    LEFT JOIN scoring_persona_configs c ON (s.persona_id = c.persona_id), wwii_player, wwii_persona 
                                    WHERE v.vehicle_id=s.vehicle_id 
                                        AND wwii_persona.personaid = s.persona_id 
...
$dtcrow = mysqli_query($dbConnComm, "SELECT DISTINCT(callsign) as callsign, streak_id, current as value2 
                                    FROM scoring_campaign_streaks s, wwii_player, wwii_persona 
                                    WHERE s.streak_id = 5
                                        AND DATE(s.achieved) >= DATE_SUB(NOW(), INTERVAL 24 HOUR)
                                        AND wwii_persona.personaid = s.persona_id
...
$allysks = mysqli_query($dbConnComm, "SELECT callsign,  
                                            s.kills as kills 
                                      FROM scoring_campaign_sorties s 
                                      ...
                                      WHERE s.persona_id=c.persona_id 
                                        AND s.country_id!='4' 
                                        AND (isnull(c.bans) OR c.bans=0) 
                                      ORDER BY kills DESC LIMIT 100 ") or die ($dbConnComm->error.kstreaksget_LINE_);
...
$dallysks = mysqli_query($dbConnComm, "SELECT callsign, 
                                            s.kills as kills 
                                       FROM scoring_campaign_sorties s 
                                       LEFT JOIN wwii_player w ON (s.player_id=w.playerid), scoring_persona_configs c
                                       WHERE DATE(s.`sortie_start`) >= DATE_SUB(NOW(), INTERVAL 24 HOUR)
...
$sortallycaps = mysqli_query($dbConnComm, "SELECT callsign, 
                                            captures as caps 
                                      FROM scoring_campaign_sorties s 
                                      LEFT JOIN wwii_player w ON (s.player_id=w.playerid), scoring_persona_configs c 
                                      WHERE s.persona_id=c.persona_id 
                                        AND (isnull(c.bans) OR c.bans=0)
                                        AND s.country_id!='4'
                                      ORDER BY caps DESC LIMIT 100") or die ($dbConnComm->error.capstreaksget_LINE_);
...
$dsortallycaps = mysqli_query($dbConnComm, "SELECT callsign, 
                                            captures as caps 
                                      FROM scoring_campaign_sorties s 
...
$axkills = mysqli_query($dbConnComm, "SELECT callsign, SUM(p.kills) AS kills 
                                        FROM scoring_campaign_personas AS p 
                                        LEFT JOIN scoring_persona_configs c ON (p.persona_id = c.persona_id), wwii_persona, wwii_player 
                                            WHERE wwii_persona.personaid = p.persona_id 
                                            AND wwii_player.playerid = wwii_persona.playerid 
...
while ($row = $axkills->fetch_assoc())
    {
       mysqli_query($dbConnComm, "INSERT INTO scoring_top_Players (side, period,killscallsign,kills) VALUES ('2','camp','".$row['callsign']."','".$row['kills']."')") or die ($dbConnComm->error.killsinsert_LINE_); //echo $row['callsign']." Has ".$row['kills']." Kills.<br>" ;
    }

/*Axis capture info 
$axcaps = mysqli_query($dbConnComm, "SELECT callsign, sum(p.captures) as caps 
                                        FROM scoring_campaign_personas p 
                                        LEFT JOIN scoring_persona_configs c ON (p.persona_id = c.persona_id), wwii_persona, wwii_player 
                                        WHERE wwii_persona.personaid = p.persona_id 
                                        AND wwii_player.playerid = wwii_persona.playerid 
                                        AND wwii_persona.countryid ='4'
                                        AND (isnull(c.bans) OR c.bans = 0) GROUP BY callsign ORDER BY caps DESC LIMIT 100") or die ($dbConnComm->error.kills_LINE_);

while ($row = $axcaps->fetch_assoc())
    {
       mysqli_query($dbConnComm, "INSERT INTO scoring_top_players (side, period,capscallsign,caps) VALUES ('2','camp','".$row['callsign']."','".$row['caps']."')") or die ($dbConnComm->error.capsinsert_LINE_);
    }
 */   
