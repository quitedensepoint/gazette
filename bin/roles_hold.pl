...
INIT
{
	if(!&addDatabase('community_db',"dbi:mysql:database=csr_community;host=localhost",'community','fun4all')){ #CP1111713 point to localhost rather than csr
...
foreach my $job (@{$jobs})
{
	if(&doUpdate('job_delete', $jobs->{'member_id'}))
	{
...
sub setPlayerDetails()
{
	...
	{
		if(!exists($player->{'clusters'}->{'reset'}))
		{
			$player->{'clusters'}->{'reset'} = &resetPlayer($cluster, $player->{'playerid'});
		}
		
		if($player->{'clusters'}->{'swapper'})
		{
